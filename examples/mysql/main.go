...
func main() {
	// Open MySQL database
	// Note: Replace with your MySQL connection details
	db, err := sql.Open("mysql", "user:password@tcp(127.0.0.1:3306)/goofer_example")
	if err != nil {
	...
	userSQL := mysqlDialect.CreateTableSQL(userMeta)
	postSQL := mysqlDialect.CreateTableSQL(postMeta)
	...
	postRepo := repository.NewRepository[Post](db, mysqlDialect)
	...
	post := &Post{
		Title:   "Hello, MySQL!",
		Content: "This is my first post with MySQL.",
		UserID:  user.ID,
	}
